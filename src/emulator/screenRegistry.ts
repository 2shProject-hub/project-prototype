...
export const SCREEN_REGISTRY: ScreenMeta[] = [
  ...
  },
  {
    ...
    label: '3-2. 브릿지 (맥락 전환)',
    category: '신규',
    description: '단어 액티비티 완료 후 문법 액티비티 진입 직전에 노출되는 맥락 연결 전환 화면. 직전에 학습한 어휘를 문법 예문의 핵심 요소로 재활용하여 단어→문법 학습 사이의 단절을 방지하고 학습 몰입도를 높인다. Spotlight Shift 패턴(배운 단어 하이라이트 → 문법 요소 순차 등장)과 Bridge Quest 패턴(배운 단어로 문장 만들기 안내)을 결합한 신규 템플릿.',
    devNotes: [
      '■ 데이터 구조 (lessonData.ts)',
      '- MOCK_BRIDGE: BridgeData 인터페이스 기반 목업 (activityNo, learnedWords, bridgeMessage, example, grammarLabel, ctaLabel)',
      '- learnedWords: 직전 단어 액티비티에서 학습 완료한 단어 목록 (프로토타입: 고정값 3개)',
      '- example.highlight: 문법 예문에 사용할 대표 단어 (직전 학습 어휘 중 1개)',
      '- example.suffix: 새로 배울 문법 요소 (예: "이에요.")',
      '',
      '■ 애니메이션 (Animated API)',
      '- 4단계 순차 페이드인: wordsOpacity → highlightOpacity → suffixOpacity → ctaOpacity',
      '- useNativeDriver: Platform.OS !== "web" 조건 분기 적용 (react-native-web 호환)',
      '- 프로토타입에서는 opacity만 사용 (transform.translateX는 web에서 불안정)',
      '',
      '■ 이식 시 주요 변경사항 (Source A ActivityLayout 연동)',
      '1. <View style={s.screen}> → <ActivityLayout ...> 로 교체',
      '2. learnedWords → ADMIN API 직전 단어 액티비티 응답 데이터로 동적 주입',
      '3. example.highlight → 학습 완료 단어 중 문법 예문 적합 단어 자동 선택 로직 추가',
      '4. onPressConfirm → 다음 문법 액티비티로 진입하는 네비게이션 처리',
      '',
      '■ 현재 프로토타입 한계 (향후 개선 필요)',
      '- 직전 화면 데이터 전달 불가: EmulatorShell의 onNavigate()가 screenId 문자열만 전달하므로',
      '  learnedWords를 직전 단어 액티비티로부터 동적으로 받을 수 없음 → 고정 mock 데이터 사용',
      '- Shared Element Transition 미구현: 단어 칩이 문장 슬롯으로 이동하는 물리적 이동 애니메이션은',
      '  react-native 기본 Animated API로 구현 불가. react-native-reanimated 도입 시 구현 가능',
      '- EmulatorShell 구조 개선 필요: onNavigate(id, payload?) 형태로 타입 확장하면',
      '  화면 간 데이터 전달이 가능해져 실제 학습 단어 연동 가능',
    ].join('\n'),
    designNotes: [
      '■ 레이아웃 구조',
      '- ActivityHeader (프로그레스바 + X버튼)',
      '- 브릿지 메시지: 20px bold, center 정렬',
      '- 단어 칩 목록: flexWrap row, teal 보더, #F0FAFA 배경',
      '- 구분선: 양쪽 line + 중앙 안내 텍스트',
      '- 문장 연출 영역: 고정 prefix(회색) + 하이라이트 칩(teal) + suffix(ink색)',
      '- 완성 문장 박스: #F0FAFA 배경, 한국어 + 베트남어 2줄',
      '- 문법 배지: tealSoft 배경, teal 텍스트, 이모지 아이콘',
      '- 하단 CTA: teal 배경, white 텍스트',
      '',
      '■ 색상 규칙',
      '- 고정 요소(저는): colors.muted (연회색) — 이미 알고 있는 요소임을 암시',
      '- 하이라이트 단어: colors.teal 텍스트 + tealSoft 배경 + 2px teal 보더 — 직전 학습 어휘 강조',
      '- 문법 요소(suffix): colors.ink — 새로 배울 내용, 대비를 통해 주목도 확보',
      '',
      '■ 애니메이션 타이밍 (참고용)',
      'Phase 1 (0ms, 400ms): 단어 칩 목록 페이드인',
      'Phase 2 (500ms, 450ms): 하이라이트 단어 칩 페이드인',
      'Phase 3 (950ms, 450ms): 문법 요소 + 완성 문장 페이드인',
      'Phase 4 (1400ms, 400ms): 문법 배지 + CTA 버튼 페이드인',
    ].join('\n'),
    sourceAFile: undefined,
