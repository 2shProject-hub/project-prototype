...
- pick(lang, ko, vi) → 다국어 매핑`,
  ...
  {
    id: 'word-text-blank',
    label: '36. 단어에 맞는 한국어 쓰기',
    category: '신규',
    description:
      '제시된 단어(한국어 또는 베트남어)를 보고, 하단에 제공된 한국어 글자 타일을 순서대로 선택하여 빈칸을 완성하는 단어 쓰기 액티비티입니다.\n\n' +

      '**1. 상단 텍스트 영역 (가이드/타이틀)**\n' +
      '- **한국어 타이틀:** "단어에 맞는 한국어를 써 보세요." (고정 문구, 굵게 표시)\n' +
      '- **베트남어 서브타이틀:** "Hãy viết tiếng Hàn phù hợp với từ dưới đây." (타이틀 하단 보조 문구)\n\n' +

      '**2. 단어 표시 카드 (Text Display Card)**\n' +
      '- 화면 중앙에 크고 눈에 띄는 카드 형태로 **정답의 기준이 되는 단어**를 노출합니다.\n' +
      '- **노출 언어 (displayLang):** Admin에서 문제별로 한국어(KO) 또는 베트남어(VI)를 선택해 등록할 수 있습니다.\n' +
      '  * **한국어(KO) 등록 시:** 카드에 한국어 단어 표시 → 학습자가 해당 한국어 단어를 글자 타일로 재조합하는 활동.\n' +
      '  * **베트남어(VI) 등록 시:** 카드에 베트남어 단어 표시 → 학습자가 의미에 맞는 한국어 단어를 글자 타일로 조합하는 활동 (번역 쓰기).\n' +
      '- **카드 디자인:** 연보라 배경, 보라색 테두리, 진보라 텍스트(28px, 굵게), 둥근 모서리 카드 형태.\n' +
      '- 보기 타일은 **displayLang에 관계없이 항상 한국어 글자**로 제공됩니다.\n\n' +

      '**3. 빈칸 영역 (Answer Slots)**\n' +
      '- **빈칸 수량:** 정답 단어의 글자 수에 따라 가변 렌더링 (Min 2개 ~ Max 7개).\n' +
      '- **빈 슬롯 상태:** 파란색(teal) 점선 테두리의 사각 박스로 표시.\n' +
      '- **채워진 슬롯 상태:** 실선 테두리 + 연한 teal 배경으로 전환되며 선택된 글자가 표시됨.\n' +
      '- **입력 방향:** 하단 보기 타일 탭 시 왼쪽 빈칸부터 순서대로 채워짐(Left-to-Right).\n' +
      '- **삭제 인터랙션:** 채워진 슬롯을 탭하면 해당 글자가 삭제되고, 뒤에 채워진 글자들이 왼쪽으로 당겨짐(Left Shift).\n' +
      '  * 예시: `[가][나][다]` 상태에서 `[나]` 탭 → `[가][다][빈칸]`으로 재정렬.\n' +
      '- **힌트 문구:** 빈칸 보드 하단에 "글자를 탭하여 빈칸에 채우세요" 안내 텍스트 표시.\n\n' +

      '**4. 보기 영역 (Character Tile Bank)**\n' +
      '- **구성:** 정답 글자 + 오답 글자(방해 타일)가 섞인 한국어 글자 타일 목록 제공.\n' +
      '- **타일 수량:** 문제별 Min 5개 ~ Max 10개 (Admin 설정값에 따름).\n' +
      '- **기본 상태:** 흰색 배경 + 회색 테두리의 사각 타일, 탭 가능.\n' +
      '- **선택(사용) 상태:** 회색 배경으로 변경, 투명도 50% 적용, 탭 불가(Disabled) — 32. 소리를 듣고 빈칸 채우기와 동일한 방식.\n' +
      '- **복구:** 채워진 슬롯에서 글자 삭제 시 해당 타일이 다시 선택 가능 상태로 복귀.\n\n' +

      '**5. 하단 [확인] CTA 버튼 정책**\n' +
      '- **비활성화(Disabled):** 빈칸이 하나라도 비어있는 상태 (초기 진입 포함).\n' +
      '- **활성화(Enabled):** 모든 빈칸 슬롯이 채워지는 순간 즉시 활성화.\n' +
      '- **재-비활성화:** 채워진 슬롯에서 글자를 삭제해 빈칸이 다시 생기면 즉시 비활성화로 복귀.\n\n' +

      '**6. 정/오답 판정 및 피드백**\n' +
      '- [확인] 탭 시 사용자가 조합한 글자와 정답 데이터를 비교해 정/오답 판정.\n' +
      '- 판정 결과에 따라 효과음(정답음/오답음) 재생 후 피드백 모달 노출.\n' +
      '- **정답 시:** 모달에 정답 단어 표시 → [다음] 탭으로 다음 문제 또는 완료 처리.\n' +
      '- **오답 시:** 모달에 오답 안내 → [다시 풀기] 탭으로 슬롯 초기화 후 재도전.\n\n' +

      '**7. 개발 및 QA 체크포인트**\n' +
      '1) **동일 글자 중복 처리:** 보기에 같은 글자가 2개 이상 있을 경우, 슬롯에서 삭제 시 고유 인덱스(Unique Tile Index) 기반으로 정확한 타일이 복구되는지 확인.\n' +
      '2) **Shift 애니메이션 연타 방지:** 슬롯 삭제 직후 빠른 연속 탭 시 배열 인덱스 꼬임이 발생하지 않는지 검증.\n' +
      '3) **displayLang별 카드 노출:** KO/VI 언어 전환에 따라 카드 텍스트가 올바르게 변경되는지 확인.\n' +
      '4) **빈칸 수 일치:** slots 배열 길이 = answer 글자 수가 항상 일치하는지 Admin 등록 단계에서 유효성 검사 필요.',
    devNotes:
      '- WordTextBlankStage\n' +
      '- WordLetterBlank 기반, AudioPlayButton → 텍스트 표시 카드 교체\n' +
      '- data: WordTextBlankQuestion[] (lessonData.ts)\n' +
      '- displayLang: ko | vi — 카드 노출 언어, tiles는 항상 한국어 글자\n' +
      '- 슬롯/타일/피드백 모달 로직: WordLetterBlank 동일\n' +
      '- kcho-dev: ActivityLayout, i18n, buildQuizItems 파서 교체 필요',
    designNotes:
      '타이틀: "단어에 맞는 한국어를 써 보세요." (bold) + VI 서브타이틀\n' +
      '텍스트 카드: speechResultBg 배경 + cultureBorder 테두리 + speechResultText 텍스트 (28px bold)\n' +
      '빈칸 보드·타일: WordLetterBlank 동일 스타일\n' +
      '하단: CtaButton 확인 (전체 슬롯 채워질 때 활성)',
    sourceAFile: 'TBD',
    sourceBRef: 'WordTextBlankStage',
  },
];
