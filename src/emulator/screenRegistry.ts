...
export const BASE_SCREEN_REGISTRY: ScreenMeta[] = [
  ...
  },
  {
    ...
    description:
      ...
      '**[기능 명세]**\n' +
      '**1. 비디오 플레이어 제어 정책**\n' +
      '- **진입 시 재생 (Auto-play):** 템플릿(액티비티) 화면 진입 완료 시 등록된 영상이 1회 자동 재생됨.\n' +
      '- **재생 중 화면 이탈:** 백그라운드 전환, 앱 최소화, 뒤로가기 시 영상 재생을 즉시 정지(Pause/Stop)하고 비디오 세션 및 메모리 해제 처리.\n\n' +
      '**2. 자막(Caption/Subtitle) 노출 정책**\n' +
      ...
      '1) **영상 리소스 누수/사운드 오버랩 방지:** [다음] 버튼 탭 시 다음 화면으로 넘어가면서 이전 영상의 오디오 트랙이 백그라운드에서 계속 재생되는 현상이 없도록 비디오 인스턴스 파기 확인.\n' +
      '2) **자막 싱크 및 다국어 렌더링:** 영상 타임스탬프와 자막 텍스트 싱크 일치 확인. 자막에 성조 기호나 특수문자가 포함될 경우 폰트 깨짐 없이 영상 하단 오버레이 영역에 안정적으로 렌더링되는지 점검.\n' +
      '3) **네트워크 지연 및 스트리밍 에러:** 동영상 로딩 버퍼링 중 또는 로딩 실패 시 플레이어 로딩 인디케이터를 노출하며, 에러 발생 시에도 하단 [다음] 버튼을 통해 사용자가 다음 액티비티로 넘어갈 수 있도록 차단 해제 보장.',
    devNotes: [
