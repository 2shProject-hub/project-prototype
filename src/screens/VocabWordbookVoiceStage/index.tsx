...
const INTRO_AUDIO = require('../../../assets/sounds/tutor_vocab_5.wav') as string;
const INTRO_TEXT = 'Trước hết, chúng mình cùng học từ vựng về tên nước và quốc tịch nhé. Các bạn hãy nghe rồi nhắc lại theo từng từ một để làm quen nha!';

export function VocabWordbookVoiceStage({ onNext, onBack }: Props) {
  const { lang } = useLang();
  const sfx = useSfx();
  ...
  useEffect(() => {
    if (!showIntro || Platform.OS !== 'web') return;
    ...
    try {
      ...
      audio.play().catch(() => {});
      audio.onended = () => setTimeout(() => setShowIntro(false), 500);
    } catch {}
  ...
  return (
    <View style={styles.screen}>
      ...
      {showIntro && (
        <View style={styles.introModalBackdrop}>
          <View style={styles.introModalSheet}>
            <Text style={styles.introModalText}>{INTRO_TEXT}</Text>
            <TouchableOpacity style={styles.introModalBtn} onPress={dismissIntro} activeOpacity={0.85}>
              <Text style={styles.introModalBtnText}>확인</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
